...
1. Create `@config-service/AGENTS.md` using
...
2. Fill in `@config-service/context/IMPLEMENTATION.md`.
   Ground it in the real project files, don't invent conventions:
   - `@config-service/src/ConfigApi.Service/ConfigApi.Service.csproj` -
     exact dependency versions
   - `@config-service/src/ConfigApi.Service/appsettings.json`
     and `.env.example` - configuration and credential handling
   - `@config-service/src/ConfigApi.Service/Models/` -
     check whether any validation attributes actually exist before
     documenting validation rules
   - `@config-service/src/ConfigApi.Service.UnitTests/coverlet.runsettings` -
